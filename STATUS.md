...
> §4.4 and in §6.1. `paper/manuscript.md` held an ordinary ASCII apostrophe the whole
...
One paper, two stated contributions. Not two papers, because the findings are causally
linked. The denominator work is what licenses the decomposition's interpretation, and the decomposition is
...
| finding | fix |
|---|---|
| `age_adjusted_rate` inner-joins after normalizing weights, so a band on one side only yields a downward-biased partial sum | guard, plus the root-cause fix below |
...
apostrophe belongs, in the abstract, in §4.4 and in §6.1. It is not a typo in the
...
- `test_the_typst_path_disables_the_writers_smart_extension` is **the one that
  ...
  prose.
- `test_the_typst_source_carries_real_punctuation` checks the mechanism on real pandoc output:
...
- **2023 is count-only corroboration.** NVSR 74-11 confirms the death count. Its rates are not
  ...
  than the per-year vintage chain, and published per 1,000 to one decimal, where one printed
  digit is worth 100 per 100,000, so it could not discriminate our 922.9 from anything in a
