...
The empirical claims in the Preamble are bound to the companion paper's findings. Until the paper's data files are populated and verified, the Preamble's factual assertions are unsupported. Section-by-section, the policy logic stands on its own; the specific magnitudes do not.
...
**(4)** Mortality during the COVID-19 pandemic was concentrated among older adults, a profile that differs fundamentally from the 1918 influenza pandemic, in which mortality peaked among young and middle-aged adults. Preparedness frameworks calibrated to one profile are poorly matched to the other.
...
**(6)** The interval between the end of a calendar year and the publication of final national mortality data routinely exceeds eighteen months, which is incompatible with the use of mortality data in active policymaking.
