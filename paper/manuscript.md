# A Fragile Equilibrium: Population Aging, Age-Specific Mortality, and the Interpretation of U.S. Crude Death Rates, {{YEAR_FIRST}}-{{YEAR_LAST}}
...
This paper decomposes the change in the U.S. crude death rate between {{YEAR_FIRST}} and {{YEAR_LAST}} into an age-specific mortality component and a population age-structure component using the Kitagawa method. Over the pre-pandemic interval {{DECOMP_PRE_INTERVAL}}, age-specific mortality contributed {{DECOMP_PRE_RATE}} deaths per 100,000 to the crude rate while the shift in age structure contributed {{DECOMP_PRE_AGE}}, a ratio of {{DECOMP_PRE_RATIO}} to one.

We further estimate excess mortality during the pandemic era against a baseline projected from the {{BASELINE_WINDOW}} trend in the age-adjusted rate, and characterize the age distribution of COVID-19 deaths, of which {{COVID_SHARE_65PLUS}} percent occurred among people aged 65 and over.
...
The other two are not zero. The Vintage 2024 restatement moved 2023's population by {{VINTAGE_POP_PCT}} percent, non-uniformly across age bands, which accounts for {{VINTAGE_SHARE_OF_DECLINE}} percent of the published 2023→2024 crude-rate decline — {{VINTAGE_RATE_SHARE}} percent of that booking as a spurious improvement in age-specific mortality. The 2010 basis moves the reported age-to-rate ratio between {{RATIO_RANGE_LOW}} and {{RATIO_RANGE_HIGH}}, which is why that quantity is reported as a range.
