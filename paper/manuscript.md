...
To determine how much of the {{DECOMP_FULL_INTERVAL}} change in the United States crude death rate reflects mortality risk rather than age structure, and whether the population denominator is stable enough to support the comparison.

**METHODS**

Deaths and population by age come from CDC WONDER, and Census Bureau vintages are used to examine the denominator. Crude rate change is decomposed using the Kitagawa method across {{AGE_BANDS_TOTAL_WORD}} age bands. Four vintage boundaries inside the series are measured against source data rather than assumed.

**RESULTS**

The crude rate rose {{DECOMP_FULL_TOTAL}} deaths per 100,000: age structure contributed +{{DECOMP_FULL_AGE}}, age-specific mortality {{DECOMP_FULL_RATE}}. Over {{DECOMP_PRE_INTERVAL}} the age effect exceeded the rate effect {{DECOMP_PRE_RATIO}} to 1. Rates rose in {{AGE_BANDS_RISING_WORD}} of {{AGE_BANDS_TOTAL_WORD}} bands. The Vintage {{YEAR_LAST}} restatement moved the 2023 population by {{VINTAGE_POP_PCT}}%, accounting for {{VINTAGE_SHARE_OF_DECLINE}}% of the published 2023-to-{{YEAR_LAST}} crude-rate decline, {{VINTAGE_RATE_SHARE}}% of which books as spurious mortality improvement.
